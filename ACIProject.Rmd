...
nzv <- nearZeroVar(aci)
```
The function recomends the removal of the `capital.gain`, `capital.loss` and `native.country` attributes. On inspection we can see that `r round((aci$capital.gain == 0)*100,2)`% of the `capital.gain` values and `r round((aci$capital.loss == 0)*100,2)`% of the `capital.loss` values are actually 0, and that `r round((aci$native.country == "United-States)*100,2)`% of the `native.country` values are "United-States".
...
Drawing this bar graph again by percentage shows that the `Self-emp-inc` group has a greater than 50% proportion over 50k - 55.87%

```{r plot_workclass_perc, echo=FALSE, message=FALSE, warning=FALSE}
aci %>% ggplot(aes(workclass, fill = income)) + geom_bar(position = "fill") + labs(y = "percent")
```

##Education

For the education attribute I used a bar graph showing the split of each education level by income, in this case using percentage straight away. 

```{r plot_education_num, echo=FALSE, message=FALSE, warning=FALSE}
aci %>% ggplot(aes(education.num, y = ..count.., fill = income)) + geom_bar(position = "fill") + labs(y = "percent")
```
...
aci %>% ggplot(aes(marital.status, fill = income)) + geom_bar(position = "fill") + labs(y = "percent") + scale_x_discrete(labels = abbreviate)
```
...
I wonder whether this might be different for each sex:

```{r plot_marital_v_sex, echo=FALSE, message=FALSE, warning=FALSE}
aci %>% ggplot(aes(marital.status, fill = income)) + geom_bar(position = "fill") + labs(y = "percent") + scale_x_discrete(labels = abbreviate) + facet_grid(sex ~ .)
```

What we see is that the proportion of female respondents earning more than 50k is even more clearly delineated across the categories with spouse present and those without. 

## Occupation

Again I plotted this on a percetnage bar graph by income

```{r plot_occupation, echo=FALSE, message=FALSE, warning=FALSE}
aci %>% ggplot(aes(occupation, fill = income)) + geom_bar(position = "fill") + labs(y = "percent") + scale_x_discrete(labels = abbreviate)
```
