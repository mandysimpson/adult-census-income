...
I considered using Quadratic Discriminant Analysis or Linear Discriminant Analysis, but discarded these approaches due to the number or predictors and the fact that most do not appear normally distributed. Instead I settle on trying a classification tree as the second approach, using cross-validation to select the complexity parameter.
...
The accuracy is higher than for the kNN algorithm - now over 82%
...
The accuracy is improved again, now to over 84%. While we cannot plot the tree in the same way as for the classification tree above, it is still possible to see the importance of each variable, measured by the Mean Decrease in Gini.
